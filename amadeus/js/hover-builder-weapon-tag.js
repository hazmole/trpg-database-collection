var HoverBuilder;
if(!HoverBuilder) HoverBuilder = {};

HoverBuilder.weaponTag = function(tag) {
  return `<div><b>${tag}</b>：</div><div style="line-height: 1.1;">${GetTagExplain(tag)}</div>`;

  function GetTagExplain(tagText) {
    const n = getLastNumber(tagText)
    if (tagText == ("接近")) return `使用此武器攻擊時，若同段落的威脅尚未行動，則只能選擇其作為攻擊目標。`;
    if (tagText == ("變幻")) return `使用此武器攻擊時，擲 1D6 隨機選擇對應段落的威脅作為攻擊目標。若對應段落沒有目標，則改為自由決定。`;
    if (tagText.startsWith("迎擊")) return `使用此武器攻擊威脅時，傷害 +${n} 點。`;
    if (tagText.startsWith("格擋")) return `使用此武器攻擊後，該輪期間的防禦判定得到 +${n} 修正。`;
    if (tagText.startsWith("必殺")) return `若使用此武器攻擊時發生大成功，傷害 +${n}D6 點。`;
    if (tagText.startsWith("精度")) return `使用此武器攻擊時的命中判定得到 ${ n<0? n: `+${n}` } 修正。`;
    if (tagText.startsWith("安定")) return `使用此武器攻擊決定傷害時，視作每顆骰都擲出了 ${n}。`;
    if (tagText.startsWith("裝甲")) return `使用此武器攻擊後，該輪期間受到的傷害減輕 ${n}D6 點。`;
    if (tagText == ("對空A")) return `使用此武器攻擊飛行狀態的目標時，命中判定得到 +1 修正，威力上升 2D6。`;
    ...
    if (tagText.startsWith("毒")) return `使用此武器造成傷害後，使攻擊目標獲得 [重傷${n}] 變調。`;
    ...
    if (tagText == ("大振")) return `使用此武器攻擊時的命中判定，選擇骰面 2 作為行動骰也會發生大失敗。`;
    if (tagText == ("野性")) return `此武器的威力上升「自己屬性的覺醒階段-1」D6 點。`;
    return "???";
  }
  function getLastNumber(str) {
    const match = str.match(/X|[+-]?\d+D?6?/);
    if (match) {
      if (Number.isNaN(Number(match[0]))) return match[0];
